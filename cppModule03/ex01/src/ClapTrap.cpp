...
ClapTrap::ClapTrap(const std::string& name): _name(name), _hp(10), _ep(10), _ad(0), _item(0){
	std::cout << "ClapTrap Constructor bot " << _name << " called" << std::endl;
}
ClapTrap::~ClapTrap(){
	std::cout << "Claptrap Destructor bot " << _name << " called" << std::endl;
}
...
void	ClapTrap::beRepaired(unsigned int amount)
{
	...
	if (this->_ep > 0 && this->_hp > 0){
		std::cout << _name << " repaired +" << amount << " health points" << std::endl;
		setHp(getHp() + amount);
		_ep--;
	}
}
